...
comp_tests(){ grep -hoE '^[begx][a-z0-9]*/[0-9]+ +([0-9]+s|.*not run)' "$BASE/shards/$1"/results/run.log* 2>/dev/null | grep -oE '^[begx][a-z0-9]*/[0-9]+'; }
...
for ((n=0;n<TOT;n++)); do
  ...
  fl=$(grep -hoE '^[begx][a-z0-9]*/[0-9]+' <(grep -hE 'output mismatch|\[failed' $logs 2>/dev/null) | sort -u)
...
if [ -n "$failed" ]; then
  ...
  if [ "${RETRY_SOLO:-1}" = 1 ]; then
    ...
    "$KERNEL" rootfstype=hostfs rootflags="$ROOTFS" rw init="/shard-init.sh" shard=retry fstyp="$FSTYP" \
      ubda="$d/dummy.img" ubdb="$d/test.img" ubdc="$d/scratch.img" \
      ubdd="$d/pool1.img" ubde="$d/pool2.img" ubdf="$d/pool3.img" ubdg="$d/pool4.img" \
      ubdh="$d/logw.img" \
      seccomp=on mem="$MEM_BIG" con0=fd:0,fd:1 con=null > "$d/boot.out" 2>&1
    solo_fail=$(grep -hoE '^[begx][a-z0-9]*/[0-9]+' <(grep -hE 'output mismatch|\[failed' "$d/results/run.log" 2>/dev/null) | sort -u)
